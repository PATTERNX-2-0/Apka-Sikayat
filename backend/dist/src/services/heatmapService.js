...
exports.getHeatmapState = getHeatmapState;
exports.calculateHotspots = calculateHotspots;
exports.startFirebaseListener = startFirebaseListener;
...
async function calculateHotspots(forceFull = false) {
    ...
    if (firebaseAdmin_1.isFirebaseAdminInitialized && firebaseAdmin_1.adminDb) {
        try {
            ...
            let queryRef = firebaseAdmin_1.adminDb.collection('complaints').where('updatedAt', '>', lastSync);
            ...
            if (!forceFull && complaintsToProcess.length > 0) {
                ...
                const fullSnapshot = await firebaseAdmin_1.adminDb.collection('complaints').get();
                complaintsToProcess = [];
    ...
    const hotspots = Object.keys(DISTRICT_COORDS).map((districtName, idx) => {
...
function startFirebaseListener(io) {
    if (firebaseAdmin_1.isFirebaseAdminInitialized && firebaseAdmin_1.adminDb) {
        console.log("[Heatmap Engine] Starting real-time Firestore listener for complaints...");
        firebaseAdmin_1.adminDb.collection('complaints').onSnapshot(async (snapshot) => {
