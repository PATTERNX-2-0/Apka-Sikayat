                  ...
                  {briefings.length > 0 && (
                    <div className="pt-4 border-t border-gray-100">
                      ...
                      <div className="space-y-2">
                        {briefings.map((b) => (
                          <div key={b.id} className="flex justify-between items-center text-xs p-2 rounded-lg bg-gray-50 border border-gray-100">
                            <span className="font-bold text-gray-700">{b.name} ({b.date})</span>
                            <button onClick={() => handleGenerateBriefing(b.type)} className="text-blue-500 hover:underline"><FileDown className="w-3.5 h-3.5" /></button>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="space-y-2 mb-4 shrink-0">
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="flex-1 px-3 py-2 rounded-xl border border-gray-200 text-sm font-bold text-gray-800"
                    />
                    <select 
                      value={visitDistrict}
                      onChange={(e) => setVisitDistrict(e.target.value)}
                      className="px-3 py-2 rounded-xl border border-gray-200 text-xs font-bold text-gray-800 bg-white"
                    >
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center px-4 py-2.5 bg-[#1E3A8A] text-white font-bold rounded-xl text-xs hover:bg-[#1E3A8A]/90 transition-all disabled:opacity-75"
                  >
                  ...
                  <div className="flex-1 overflow-y-auto space-y-4 pr-1 custom-scrollbar">
                    <div className="grid grid-cols-2 gap-2 text-center">
                      <div className="p-2 bg-gray-50 rounded-xl border border-gray-100">
                        <span className="text-[8px] font-black text-gray-400 uppercase tracking-wider block">Total Complaints</span>
                        <span className="text-base font-black text-gray-800">{visitStats.total}</span>
                      </div>
                      <div className="p-2 bg-red-50/50 rounded-xl border border-red-100">
                        <span className="text-[8px] font-black text-red-500 uppercase tracking-wider block">Critical Case</span>
                        <span className="text-base font-black text-red-700">{visitStats.critical}</span>
                      </div>
                    </div>
                    ...
                    {visitSpeech && (
                      <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl">
                        ...
                        <p className="text-xs font-bold text-blue-900 leading-relaxed line-clamp-3 mb-2">{visitSpeech}</p>
                        <div className="flex justify-end gap-2">
                          <button onClick={() => handleDownloadVisitPDF('PDF')} className="flex items-center text-[10px] font-black uppercase tracking-wider text-blue-600 bg-white border border-blue-200 px-2.5 py-1.5 rounded-lg hover:bg-blue-50 transition-colors">
                            <Download className="w-3 h-3 mr-1" /> Briefing
                          </button>
                          <button onClick={() => handleDownloadVisitPDF('SPEECH_PDF')} className="flex items-center text-[10px] font-black uppercase tracking-wider text-blue-600 bg-white border border-blue-200 px-2.5 py-1.5 rounded-lg hover:bg-blue-50 transition-colors">
                            <Download className="w-3 h-3 mr-1" /> Speech PDF
                          </button>
                        </div>
                      </div>
                    )}
                ...
                <div className="space-y-3 overflow-y-auto custom-scrollbar pr-1 flex-1">
                  {audits.map((off, i) => (
                    <div key={i} className="p-3 border border-red-100 bg-red-50/30 rounded-xl">
                      ...
                      <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider block mb-1.5">{off.dept} • Score: {off.riskScore}</span>
            ...
            {activeSidebar === 'POLICY' && (
              <motion.div key="policy" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="p-5 flex flex-col h-full">
                ...
                <div className="space-y-4 overflow-y-auto custom-scrollbar pr-1 flex-1">
                  {policies.map((p, idx) => (
                    <div key={idx} className="p-4 border border-gray-200 rounded-xl hover:border-[#FF9933]/50 transition-colors cursor-pointer group">
                      <div className="w-8 h-8 rounded-lg bg-[#FF9933]/10 flex items-center justify-center text-[#FF8C00] mb-3 group-hover:scale-110 transition-transform">
                        {p.type === 'workforce' ? <Users className="w-4 h-4" /> : <BarChart3 className="w-4 h-4" />}
                      </div>
                      <h4 className="font-black text-gray-900 text-sm mb-1">{p.title}</h4>
